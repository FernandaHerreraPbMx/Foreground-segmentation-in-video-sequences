...
{																						// TH and TS as hue and saturation thresholds
	if(!_rgb){
		...
		hsvBKG.convertTo(hsvBKG,CV_64FC3);
		split(hsvBKG, chansBKG);
		chansBKG[0] = chansBKG[0] * 2;
		chansBKG[1] = chansBKG[1] * (1/2.55);
		chansBKG[2] = chansBKG[2] * (1/2.55);

		cvtColor(_frame, hsvFRAME, COLOR_BGR2HSV);
		hsvFRAME.convertTo(hsvFRAME,CV_64FC3);
		split(hsvFRAME, chansFRAME);
		chansFRAME[0] = chansFRAME[0] * 2;
		chansFRAME[1] = chansFRAME[1] * (1/2.55);
		chansFRAME[2] = chansFRAME[2] * (1/2.55);

		divide(chansFRAME[2], chansBKG[2], t1,1);							     		// Value channel
		...
		threshold(t2, t2, TS, 255, 1);													// Satutation channel compared against its thresholds
		threshold(t3, t3, TH, 255, 1);													// Hue channel compared against its thresholds

		t1.convertTo(t1,CV_8UC1);
		t2.convertTo(t2,CV_8UC1);
		t3.convertTo(t3,CV_8UC1);
	...
	_bgsmask.copyTo(_shadowmask,255-_bgsmask);
